...
核心压缩、解压、密码识别、分卷、取消、事务式输出和更新签名流程已有自动化回归，v1.0.13 Release 与最新 `master` CI 均成功，当前没有已知的发布阻断级代码故障。`master` 已启用 PR、四项 CI、对话解决、禁止强推和禁止删除保护；发布验收流程已沉淀到 [RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md) 和发布验收 Issue 表单。AES 流式 v2 已建立格式规范、统一分块内核和 v1 只读兼容，后续仍需接入任务取消及性能基准。Windows 11 顶层右键菜单依赖可信代码签名证书，当前明确暂缓。
...
1. 为 AES v2 分块边界接入任务取消，并增加 100MiB/1GiB 峰值工作集基准；`AESENC01/TARAES01` 保持受限的只读兼容。
