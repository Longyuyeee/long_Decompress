...
[![Version](https://img.shields.io/badge/version-1.3.6-6366f1?style=flat-square)](https://github.com/Longyuyeee/long_Decompress/releases/latest)
...
- 保存新任务不再静默删除第 500 条之外的历史，保留到用户主动删除或清空；记录会占用本机磁盘，旧版已删除的数据无法恢复。
- 新增“加载更早记录”，使用稳定游标分页；加载失败保留现有列表，可再次尝试。
- 明确筛选、统计和趋势只覆盖当前已加载记录，全库搜索尚未实现；刷新/清空时不会被迟到的查询结果覆盖。
- [发布说明与下载](https://github.com/Longyuyeee/long_Decompress/releases/tag/v1.3.6) · [本版审计](long-compress-assistant/docs/RELEASE_AUDIT_1.3.6.md) · [下一版计划](long-compress-assistant/docs/NEXT_VERSION_AFTER_1.3.6.md)。
...
- 视频/PDF 切换页面后仍可取消原批次，规划与历史保存期间防止重复启动。
- PDF 文件列表、已确认配置和输出目录在本次运行中保留；离开页面会清空未提交密码，不自动恢复或执行任务。
- 修复视频结果验证期间取消后 Windows 文件句柄尚未释放导致的暂存残留。
- [发布说明与下载](https://github.com/Longyuyeee/long_Decompress/releases/tag/v1.3.4) · [本版审计](long-compress-assistant/docs/RELEASE_AUDIT_1.3.4.md) · [下一版计划](long-compress-assistant/docs/NEXT_VERSION_AFTER_1.3.4.md)。

### v1.3.3 解压诊断、历史找回与跨页面任务控制

- 损坏或截断 ZIP 给出中文最终原因；同名文件等待选择时不再提前记作失败。
- 历史筛选无结果可重置找回已加载记录；从历史创建新解压草稿，保留原失败记录且不自动执行。
- 压缩中心取消/清除仅影响归档，尊重未保存历史；图片批次切换页面后仍保留进度和取消控制。
- [查看 v1.3.3 发布说明](https://github.com/Longyuyeee/long_Decompress/releases/tag/v1.3.3)。
- [发布审计与验证边界](long-compress-assistant/docs/RELEASE_AUDIT_1.3.3.md) · [下一版开发顺序](long-compress-assistant/docs/NEXT_VERSION_AFTER_1.3.3.md)。
...
- 重构解压任务摘要、折叠行与展开详情，控制长路径和指标换行，精简存储预检，并修复终态异常速度；显示百分比增加平滑过渡。
- Windows 卷根目录的事务暂存固定落在目标卷，避免最终发布时触发跨卷 `os error 17`。
- 标题栏拖动和八方向缩放改由 Windows 原生非客户区循环处理；真实系统输入验证窗口移动、缩放及松键后停止。
- 920×620、760×520 的真实 Tauri 响应式门禁与隔离卷根产品 IPC 解压均已通过。
- [查看 v1.2.6 发布说明](https://github.com/Longyuyeee/long_Decompress/releases/tag/v1.2.6)

### v1.1.13 解压主流程可信度修复

- 密码保险箱、导入词表和内置字典尝试期间，解压进度严格保持 0；只有引擎实际开始写出内容后才显示字节进度。
- 修复正确 RAR 密码可能因固定短超时被误判的问题，并以固定官方加密 RAR 验证错误密码拒绝、正确密码解压和逐文件 SHA-256。
- 事务暂存目录创建后立即隐藏并在清理后刷新 Explorer；文件冲突复用同一份暂存逐项提交，不再从头重复解压。
- Windows 容量预检增加原生后备，新任务历史保存实际识别格式，长密码候选日志保留关键阶段并有界采样。
- [查看 v1.1.13 发布说明](https://github.com/Longyuyeee/long_Decompress/releases/tag/v1.1.13)
...
| 压缩包浏览 | 不必完整解压即可查看目录树、搜索和筛选文件，支持默认应用安全打开、图片/文本受限预览、最多三层只读嵌套浏览和精确选择后解压 |
...
- ZIP/TAR 系列中的受支持图片和文本可进行有界只读预览；预览受解压体积、像素数和扫描预算限制。
- 双击归档内的压缩包可进入最多三层的只读子工作区；每层密码独立，顶部归档链可返回并恢复外层目录、筛选和选择状态。
