...
- 标签发布同期的普通 CI 还发现两项跨环境门禁漂移：浏览器模拟未返回后端新增的图片预览能力，旧断言又错误排斥已交付的“默认应用打开”；Rust 1.98 新增 Clippy 规则则拒绝固定宽度 `chunks_exact(2)`。现已让模拟能力与真实后端结构一致、同时断言两种打开方式，并采用兼容旧工具链的 UTF-16 成对遍历。Chromium 9/9、真实预览 Rust 12/12 与全目标 Clippy 严格检查均通过；这次发布后提交不移动标签、不改变公开二进制。
