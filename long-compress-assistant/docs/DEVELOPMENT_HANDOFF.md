...
- 实际代码确认 D-01 和 D-02.1 已关闭：产品现在注册 qpdf 固定参数的结构化只读分析，但压缩页仍是 PDF 占位区域，没有转换命令。下一唯一功能接续点是 D-02.2 两模式配置和执行前风险确认。
...
- D-01 与 D-02.1 现已关闭，但 PDF 转换命令和 UI 仍冻结。下一唯一接续点为 D-02.2 两模式配置与风险界面，详见 [D02_1_PDF_READONLY_ANALYSIS_AUDIT.md](D02_1_PDF_READONLY_ANALYSIS_AUDIT.md)。
