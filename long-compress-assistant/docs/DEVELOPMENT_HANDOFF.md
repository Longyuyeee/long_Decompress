...
- 当前正式基线仍为 `v1.1.13`；本轮只完成路线审计与工作项拆分，不升版本、不打包发布。
- 压缩包浏览中心 2.0 为第一大节点：先完成聚焦/多选语义、双击、右键菜单、默认应用安全打开、会话缓存和嵌套归档，再执行真实 Windows 桌面闭环；完整后建议发布 `v1.1.14`。
...
- 本轮聚焦验证：浏览中心前端 5/5 通过；归档相关 Rust Release 测试 38 项通过、1 项外部真实分卷门禁按条件忽略，相关格式矩阵通过。本机桌面门禁因缺少 Microsoft EdgeDriver 未启动，节点 A 开始前必须补齐匹配 WebView2 的驱动或在已配置 Windows runner 取得新证据。
