...
- 本地旧机 DPAPI updater 密码不可解密，签名资产只能由 GitHub Actions Secrets 生成；在工作流、四项公开资产、`latest.json` 和公开更新复验全部通过前，不得把候选描述成已发布。
- 下一步将候选快进到 `master`、创建 `v1.1.14` 标签，等待 Release 工作流后执行公开资产和更新门禁。完整证据见 [RELEASE_AUDIT_1.1.14.md](RELEASE_AUDIT_1.1.14.md)。
