...
- 候选身份已提升为 `1.0.20-rc.1`，八处版本来源和版本化 Shell DLL 一致。不含 E2E 桥的生产 NSIS 候选已从
  公开 `1.0.17` 完成覆盖安装、原目录保持、两套用户数据指纹保持、传统菜单注册、卸载清理和基线恢复；
  42 项真实安装检查全部通过。验收脚本会备份并精确恢复应用自有右键注册表树，预发布版本 DLL 名称也按统一规则校验。
...
2. 原生解压、计划内写入职责、磁盘写满及 `1.0.20-rc.1` 覆盖安装/卸载/数据保持验收已经收口；
   下一步完成候选安装版的核心归档与界面人工抽查，等待 PR 门禁通过后合入 `master`。
...
5. 正式发布时将候选身份收敛为 `1.0.20`，重新生产打包并更新 README 与 Release；发布后立即从保留的
   上一正式版执行 `test:public-update`，回填应用内更新和自动重启证据。
