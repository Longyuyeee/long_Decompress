...
## 2026-09-12 原生文件选择器复验（当前暂停点）

- 已拉取 `master@c6e1ffc`，工作区干净；核对历史草稿、解压启动及后端文件/分卷响应代码，未为验收修改产品或断言。版本保持 1.3.2。
- 测试程序 SHA-256 与上轮最终构建一致（`7DABB4623D318D3FF421E2B6CE540FB8CAEC160B035DBB2BFEEE68B6A9D74E33`），本次重新生成独立资料：`test-results/failure-user-scenario-YApA7n`，独立历史库为其下 `app-data`。
- 真实操作再次到达“选择压缩包”，但控件点击报 `coordinate input geometry is unavailable`；重新定位并恢复前台后，选择器截图点击报 `unknown screenshotId screenshot-2`。按 computer-use 恢复规则停止输入，没有以后台注入任务替代原生选文件。
- 当前等待用户在已打开的隔离测试程序里手动选择该目录的 `下载中断.zip`。选择完成后重新观察窗口，从“开始解压”继续；不得假定文件已选入，也不得假定后续桌面操作一定可用。程序暂留，若进程已关闭则按独立数据目录重新启动，不操作用户正式历史库。
- 本次没有新增通过的解压场景，也未重复执行全量单测；上轮 372/372 仍仅是上轮证据。后续按“截断包失败原因→完整包成功与内容核对→历史创建新任务→重启保留记录”继续，保持原验收标准。不要为了绕过工具问题新增产品入口或改变窗口逻辑。

## 2026-09-12 压缩清除一致性（最新接续点）

- 两步修复已随 PR #137 合入 `master@c9fd64a`；最终代码 `e5cec5c` 的 Windows CI 34680346092 通过（5m38s），此后仅补录证据。
- 用户场景审计后继续修复：压缩中心先尊重任务仓库的历史保存保护，再同步移除文件行；保存中/失败时两处均保留并说明原因。
- 三条用户口吻场景在旧逻辑上复现行丢失、修复后通过；旧“已结束清除”测试保留断言并等待真实保存 Promise。正式完整单测现在 372/372、定向 52/52、生产构建通过。
- 详见 [压缩清除一致性](COMPRESSION_HISTORY_CLEAR_CONSISTENCY_2026-09-12.md)。下节 368/369 为本轮第一步的中间结果，不是最终状态。
- 桌面文件选择器定位故障尚未解决，真实 UI 闭环不记为通过；下一步补完整包/截断包/重新下载后的新任务链。版本仍为 1.3.2，未发 Release。

## 2026-09-12 用户操作顺序审计（最新接续点）

- 历史详情先展示最终失败原因，再提供核对/导出与新草稿入口；新增用户口吻场景先红后绿，未削弱验收。
- 定向回归 88/88、Rust 诊断 12/12、生产构建通过；完整单测 368/369，压缩中心清除与历史保存时序仍有 1 项失败，不能宣称全部收尾。
- 真实桌面只到原生文件选择器，computer-use 定位恢复失败，未完成文件提交或失败历史闭环，不算验收通过。隔离测试进程已关闭。
- 下一步先修压缩工作项清除与任务保存保护不同步，再补完整/截断/重新下载三条真实 UI 链。见 [用户场景审计](USER_SCENARIO_AUDIT_2026-09-12.md)。版本仍为 1.3.2，未发布 Release。

## 2026-09-12 回滚人工核对指引（最新接续点）

- PR #136 已合入 `master@777c002`，最终代码提交 `d166b74` 的 Windows CI 34679686559 通过；本轮完成审计、测试与推送收尾，未发布新版本。
- 历史详情与诊断报告统一显示回滚不完整后的核对步骤；不猜测恢复路径，不自动操作文件。原有历史草稿入口保留并明确不是恢复旧任务。
- 扩大前端回归 85/85、生产构建通过，版本仍为 1.3.2。见 [回滚核对指引](ROLLBACK_RECOVERY_GUIDANCE_2026-09-12.md)。
- 下一步建议综合审计近期失败/回滚/历史补强并收敛补丁范围，自动恢复和逐文件映射仍未实现。

## 2026-09-12 回滚不完整保留数据（最新接续点）

- PR #135 已合入 `master@922f49a`，最终提交 Windows CI 34679288599 通过，本轮完成收尾。
- 回滚不完整使用独立错误并记录恢复目录；普通解压禁用自动清理，冲突恢复移出 pending，不能继续提交同一份不完整暂存。未实现自动恢复。
- 事务 17/17、回滚保护 2/2、历史 12/12、前端 97/97、类型检查、生产构建及严格 Clippy 通过；版本仍为 1.3.2。重复确认不会覆盖原始恢复路径。
- 详见 [回滚不完整保护](INCOMPLETE_ROLLBACK_RETENTION_2026-09-12.md)，上轮两项清理/重试风险已处理，下一步补人工核对信息与恢复计划设计。

## 2026-09-12 解压回滚恢复暂存（最新接续点）

- PR #134 已合入 `master@da3f7bc`，Windows CI 34678795268 通过，本轮成功回滚后的暂存恢复修复完成收尾；下述回滚不完整事项仍待处理。
- 修复部分提交失败后回滚删除新文件却未恢复暂存，导致重试可能缺文件的问题。成功回滚现在恢复暂存与原目标，Windows 实际占用后重试验证通过。
- Rust 事务 16/16、前端 54/54、严格 Clippy 通过，版本仍为 1.3.2。
- 见 [回滚暂存完整性](EXTRACTION_ROLLBACK_STAGING_2026-09-12.md)。回滚不完整时的 pending 重试资格与残留备份保留仍是下一优先项，不能承诺该状态可直接重试。

## 2026-09-12 解压已提交后的清理告警（最新接续点）

- PR #133 已合入 `master@7dd2f22`，生产构建与 Windows CI 34678368063 通过，本轮完成收尾。
- 修复冲突恢复路径在提交成功后因临时目录清理失败而误报任务失败的问题；与普通解压统一为警告并继续完成，不重跑解压。
- Windows 实际占用测试通过；Rust 事务 14/14、历史 12/12、前端 54/54、类型检查和严格 Clippy 通过。版本仍为 1.3.2。
- 详见 [已提交解压清理告警](COMMITTED_EXTRACTION_CLEANUP_2026-09-12.md)。下一步继续解压提交失败与冲突恢复的错误分类。

## 2026-09-12 单文件压缩输出提交失败（最新接续点）

- PR #132 已合入 `master@ed36cd4`，Windows CI 34677986373 通过；追加校验失败回归 3/3 通过，本轮完成收尾。
- 单文件压缩输出提交区分目标冲突和其他提交失败，保存 Publishing 阶段、原始详情及 I/O 原因链；取消、校验错误及既有清理策略保持原语义。
- 前端 102/102、Rust 提交/事务/历史/磁盘回归、类型检查、构建和严格 Clippy 通过，版本仍为 1.3.2。
- 范围、验证及下一步见 [输出提交失败](OUTPUT_PUBLICATION_FAILURE_2026-09-12.md)。下一步继续解压提交/冲突恢复，不能视为全部路径已完成。

## 2026-09-12 压缩产物校验失败分类（最新接续点）

- PR #131 已合入 `master@ae23db6`，最终代码 Windows CI 34677581252 通过。审计中补修磁盘已满归一化丢失校验阶段的问题，补充磁盘写满 3/3 回归通过，本轮完成收尾。
- 校验失败保存明确类别与 Verifying 阶段，保留原始详情，不武断判定损坏；取消保持取消，不误入密码重试。
- 前端 98/98、Rust 历史 12/12、服务校验回归 3/3、原生校验 4/4，类型检查、构建与严格 Clippy 通过。版本 1.3.2、schema 8 不变。
- 详见 [压缩产物校验失败](OUTPUT_VERIFICATION_FAILURE_2026-09-12.md)。下一步接输出提交，保留失败、清理警告和冲突等待的区别。

## 2026-09-12 源文件检查失败分类（最新接续点）

- PR #130 已合入 `master@1160c87`，Windows CI 34677120465 通过并上传安装器，本轮完成收尾。
- 纠正入口将目录/检查异常统一报为文件不存在的问题；源缺失、无法检查、不是普通文件均保存明确类别与预检/解压阶段，并排除误入密码重试。
- 前端 96/96、Rust 27/27、类型检查、生产构建及严格 Clippy 通过；版本 1.3.2、schema 8 不变。
- 详见 [源文件检查失败](SOURCE_ACCESS_FAILURE_2026-09-12.md)，下一步继续校验和输出提交错误，尚未实现项不提前标记完成。

## 2026-09-12 源归档变化分类（最新接续点）

- PR #129 已合入 `master@00719ff`，Windows CI 34676741608 通过；补充解压事务回归 13/13 通过，本轮完成收尾。
- 预检/密码检测后及解压结束后的源变化已接入明确类别和阶段；历史显示“源文件已变化”，不再仅为未分类，也不会误入前端密码重试。
- 版本仍为 1.3.2，schema 仍为 8；前端 90/90、Rust 历史 12/12、真实临时文件增长 1/1、类型检查、构建和严格 Clippy 通过。
- 实现边界、证据及后续源读取/校验/输出错误方向见 [源归档变化分类](SOURCE_CHANGE_FAILURE_2026-09-12.md)。

## 2026-09-12 结构化失败历史（最新接续点）

- PR #128 已合入 `master@032ab2c`；扩大联动回归 72/72、生产构建及 Windows CI 34676326862 均通过，本轮完成收尾。
- 新失败类别、阶段、判断依据进入 schema 8 历史数据库；旧数据不回写。检测标记优先，其他阶段明确为“最后观测阶段”。
- 历史详情、筛选和诊断导出优先读取持久化结构，旧记录保持兼容；仍为 1.3.2。
- 定向前端 27/27、Rust 历史 12/12、迁移 4/4 和严格 Clippy 通过；后续继续非检测阶段类型化错误、配置快照和跨进程未保存记录恢复，不宣称全部错误模型收口。
- 详见 [结构化失败历史](STRUCTURED_FAILURE_HISTORY_2026-09-12.md)。

## 2026-09-12 从历史创建完整解压草稿（最新接续点）

- PR #127 已合入 `master@a857e27`；定向 43/43、类型检查、生产构建和 Windows CI 34675816882 均通过，本轮已收尾。
- 历史解压详情新增草稿入口，可换源文件、确认输出目录，复用分卷检测并检查来源为非空文件。
- 原配置不足以原样恢复，因此明确创建“完整解压”待执行任务：新 ID、单独配置、同名子目录、保留源文件；不恢复密码/筛选，不自动执行，原历史不改写。
- 关联历史 ID 进入草稿日志；关闭面板不遗留异步创建。当前版本仍为 1.3.2。实现与测试见 [历史解压草稿](HISTORY_EXTRACTION_DRAFT_2026-09-12.md)。

## 2026-09-12 历史错误检索与诊断报告（最新接续点）

- PR #126 已合入 `master@c1876ea`，Windows CI 34675400669 通过并上传安装器，本轮完成收尾。
- 已完成基于已保存检测标记的类别/阶段描述、历史错误全文检索与类别筛选、单任务 JSON 诊断报告导出。
- 原因缺失或旧字符串不猜测分类，历史数据库不回写；本轮是读取层兼容增强，不代表全引擎结构化错误落库完成。
- 类型检查和聚焦 25/25 测试通过，版本保持 1.3.2；后续接统一错误输出、任务重建和跨重启未保存历史恢复。详见 [历史诊断实现与验证](HISTORY_DIAGNOSTICS_2026-09-12.md)。

## 2026-09-12 错误分类与历史保存补强（最新接续点）

- PR #125 已合入 `master@08451e5`；最新 PR 提交的 Windows package 检查通过，安装器产物已上传（CI 34674642329）。本轮实现、审计、推送和打包检查已完成。
- 已修正 RAR 检测失败一律标损坏、缺引擎判未加密、混合错误优先试密码，以及前端再次扫描原始详情误入密码提示的问题。
- 类型检查、前端 31/31 和 Rust 聚焦回归通过；真实随包引擎已验证普通/加密 RAR 与独立截断副本。
- 同任务历史保存已串行，增加保存失败提示与只补写历史的重试；未保存任务保留到保存成功后再清理。合并范围定向回归 50/50、生产前端构建通过。
- 下一步是兼容的结构化失败类别/阶段和历史错误检索/诊断报告；不会把本轮过渡标记写成完整错误模型已交付。仍为 v1.3.2 后续开发，未升版发布。证据见 [失败可靠性补强](FAILURE_RELIABILITY_FOLLOWUP_2026-09-12.md)。

## 2026-09-12 当前审计与升级规划（最新接续点）

- 已拉取，基线仍为 `master@51b6767`，正式版本 v1.3.2。当前处于已发布产品补强阶段。
- 新审计发现 RAR 检测异常统一标损坏、混合密码提示优先、引擎缺失视作未加密等分类缺口；历史保存乱序为待复现风险。下一补丁优先处理错误分类与终态保存，然后接历史诊断和重试操作。
- 本轮类型检查与任务/命令/历史 29/29 定向测试通过；未复跑旧真实样本和全量桌面矩阵，未改变产品代码或版本。
- 根 README 当前版本已纠正为 v1.3.2。完整代码证据、竞品来源、后续六步规划与验收见 [当前审计与升级规划](DEVELOPMENT_AUDIT_AND_UPGRADE_PLAN_2026-09-12.md)；下方各“最新接续点”为当时的历史快照。
...
- 用户真实近期记录表明，一批 RAR 在仍为空文件或继续写入时进入解压；同时保险箱候选已被记为使用，但完整解压随后错误进入手动密码重试。
- 根因已在 v1.3.0 实际代码中关闭：RAR 候选只由加密条目证明；密码已被证明后，后续 `BadData` / `BadPassword` 不再回退成密码错误；源文件在预检或解压期间变化会清理暂存并提示等待写入完成；保险箱使用次数后移到完整载荷解压成功。
- Rust 主库 389/389、RAR 聚焦 3/3、固定真实加密 RAR 往返、自动密码来源 6/6 和严格 Clippy 均通过。本轮未升版、打包、打标签或创建 Release；下一步如需交付安装版，应独立提升补丁版本并走发布闭环。
- 完整证据见 [RAR_PASSWORD_POST_VALIDATION_FIX_AUDIT_2026-09-05.md](RAR_PASSWORD_POST_VALIDATION_FIX_AUDIT_2026-09-05.md)。

## 2026-09-04 v1.3.0 发布候选（最新接续点）

- `v1.2.9..master` 的 16 个提交已完成集中审计，范围包括归档配置布局、任务按钮与全文浮窗、特殊压缩按钮对齐、可输入文件路径、密码显示/任务密码信息、密码生成器、分卷配置、压缩密码保险箱联动、完整性工作区和压缩包选择筛选。
- 合并后前端回归为 56 个文件、314/314 项通过；类型检查和生产构建通过。分卷配置、密码下拉、文件选择过滤与文件完整性流程均有独立回归覆盖。
- 版本身份统一提升为 `1.3.0`；版本化 Shell 扩展必须为 `long_compress_shell_extension_1_3_0.dll`，发布标签必须为 annotated `v1.3.0`。
- 正式 Release 由标签触发的 Windows workflow 构建 NSIS、updater ZIP、签名与 `latest.json`。工作流完成前不得把本机前端构建写成安装器已发布；公开资产状态以 GitHub Release 为准。
- 发布说明见 [RELEASE_NOTES_1.3.0.md](RELEASE_NOTES_1.3.0.md)。
...
- `v1.2.6` 后续两轮布局修复已经通过需求审计，版本身份提升到 `1.2.7`；正式发布前仍属于候选。
- 候选范围包括解压任务名称/状态/自然排序/存储预检/低高度版本标识、全局省略文本完整提示、三个核心工作区密度和窄窗口固定左右双栏。
- 已有功能证据：前端单元 52 文件 290/290、Chromium 13/13、生产构建、真实 Windows Tauri 文件浏览器及 920×620 / 760×520 响应式门禁通过。发布级 Rust、版本身份、Shell 扩展、CI、tag、Release 与公开资产回下载仍需按顺序闭环。
- 候选说明见 [RELEASE_NOTES_1.2.7.md](RELEASE_NOTES_1.2.7.md)，发布审计见 [RELEASE_AUDIT_1.2.7.md](RELEASE_AUDIT_1.2.7.md)。

## 2026-09-03 v1.2.6 后续工作区密度与双栏修复（当前接续点）

- 当前分支 `codex/decompression-table-layout`；本轮仍是公开 `v1.2.6` 后续修复，没有升版、打包、打标签或创建 Release。
- 已关闭三项需求：所有真实省略的单行文本统一在悬停/聚焦时提供完整 `title`，且不覆盖已有业务标题；解压、压缩、特殊压缩标题与上下留白向文件浏览器的紧凑密度对齐；双栏文件浏览器在 920、760、390 px 下始终保持左右排列，不再切为上下布局。
- 真实 Windows Tauri 已在 920×620 / 760×520 逐张截图审计：双栏顶边对齐、右栏位于左栏右侧、根容器无横向溢出；解压与压缩标题更靠上、底部操作区更靠下。用户正在运行的正式版没有被停止或替换。
- 类型检查通过；前端单元 52 文件 290/290；Chromium 13/13；E2E Release 构建、真实 Windows 双栏文件管理门禁和响应式任务详情门禁均通过。完整证据见 [WORKSPACE_DENSITY_AND_DUAL_PANE_AUDIT_2026-09-03.md](WORKSPACE_DENSITY_AND_DUAL_PANE_AUDIT_2026-09-03.md)。
- 下一步先提交、推送并等待 CI；若继续界面优化，可单独处理 760×520 下压缩任务展开详情的信息密度。没有新的发布授权时，不提升版本、不覆盖 README 发布信息、不创建 Release。

## 2026-09-03 v1.2.6 后续解压任务布局修复（当前接续点）

- 当前分支 `codex/decompression-table-layout`，基线为公开 `v1.2.6` 后续的 `master@438bfad`；本轮没有升版、打包、打标签或创建 Release。
- 五项需求已经关闭：任务名称列加宽且省略名称可悬停查看全文；状态区改为固定 Grid 轨道并让终态居中；多文件按中文数字自然名称排序；存储预检恢复位置/介质/剩余/预计占用/文件系统/预留信息；低高度侧栏重新显示版本徽标。
- 两轮真实界面纠偏均已保留：先修正 920×620/760×520 配置栏 245/248 px 的宽度不足，再发现 760×520 预检卡被底部操作区遮挡并将其前移。最终真实 Windows Tauri 两档响应式门禁通过，760×520 截图中预检卡完整可见、版本号为 `v1.2.6`。
- 真实图片基线 11 张 + 1 个 PDF 拒绝边界、前端单元 51 文件 288/288、Chromium 12/12、生产构建和真实 Windows Tauri 响应式门禁均通过。没有停止或替换用户正在运行的正式版。
- 下一步如继续功能开发，先用真实长中文名、自然编号、加密/非加密混合归档观察活动态；如决定升版，必须另走完整 Rust、安装生命周期、版本身份、NSIS、CI、README、tag、Release 和公开资产回下载闭环。完整证据见 [DECOMPRESSION_TASK_LAYOUT_AUDIT_2026-09-03.md](DECOMPRESSION_TASK_LAYOUT_AUDIT_2026-09-03.md)。

## 2026-09-03 v1.2.6 正式发布关闭（历史基线，已由上方节点取代）
...
- `v1.2.6` 已由 PR #119 的五项 CI 全绿后合入 `master@0e04574`；annotated tag `v1.2.6` 指向该提交。Release run `33714184134` 全绿，正式 Release 和安装包、更新 ZIP、签名、`latest.json` 四项公开资产已经发布并回下载验证。
- 当前公开稳定版为 `v1.2.6`。下一次开发从最新 `master` 开始；本机安装生命周期没有执行，因为会停止并替换用户正在运行的正式版，不能把该项写成已通过。Windows 11 第一层菜单仍等待生产代码签名身份，Windows N 仍不在支持范围内。
...
## 2026-09-03 过期测试/文档清理与发布判定（历史，已由上方节点取代）
...
- 当前实现由标题栏空白区显式调用 `appWindow.startDragging()`，窗口按钮不会触发拖动；缩放会在读取原生指标前注册结束监听，并由 `pointerup`、`pointercancel`、窗口失焦、capture 丢失及 `buttons` 状态共同收口，物理位移按 DPI 换算为逻辑像素；高度不超过 640 px 时 8 个导航项使用 32 px 高、2 px 间距，总高度 283 px，无滚动条且全部可见。
- 已通过：类型检查；前端 49 文件 286/286 单元测试；Playwright 40 通过、20 条件跳过、0 失败；生产构建；发布身份；`cargo check`；严格 Clippy；Rust 主库 384/384（10 项显式忽略）及全部集成测试；隔离 Release Tauri/WebView2 壳门禁。真实桌面证据中缩放增量为 16×12 逻辑像素（本机 250% DPI），松键回入后尺寸不变，侧栏 `clientHeight=scrollHeight=283`、8/8 可见，expected/actual 差异为 0。
- 自动化驱动的合成指针不具备 Windows 真实左键状态，无法可靠证明系统窗口物理移动；门禁已验证真实 Release WebView2 收到标题栏 `mousedown`、Cargo/allowlist 能力存在并可编译，单元测试验证只从空白标题栏发起原生调用。换机或打包前仍应在候选应用上用真实鼠标完成一次标题栏移动体验确认，不得把合成指针写成物理拖动证据。
- 下一步：推送本分支并等待 CI；若准备修复版发布，再单独决定版本号并执行候选 NSIS、安装生命周期、真实鼠标体验、PR 合并、标签、Release、资产回下载和公开更新闭环。详细审计见 [WINDOW_INTERACTION_AND_SIDEBAR_AUDIT_2026-09-02.md](WINDOW_INTERACTION_AND_SIDEBAR_AUDIT_2026-09-02.md)。

## 2026-09-02 v1.2.4 发布候选（历史，已正式发布）

- 当前分支为 `codex/special-compression-shell-polish`，基线是公开 `v1.2.3` 后续文档提交 `master@7a21fea`；版本身份已提升为 `1.2.4`，但在正式构建、CI、标签和 GitHub Release 全部结束前仍只是候选。
- 已完成：删除特殊压缩泛化提示；视频默认折叠并仅显示核心摘要；图片/视频弹窗缩至 `lg`，PDF 输出配置迁入紧凑弹窗；压缩中心全局设置改为紧凑两列，配置组使用 736×576 上限、固定标题和主题化滚动区；恢复无原生装饰窗口的显式拖动和八方向缩放；删除顶端重复产品名；侧栏隐私卡替换为固定任务槽；全局滚动条统一为 5 px 低对比度样式且隐藏原生箭头。
- Windows Release Tauri 壳门禁实际结果与预期差异为 0：无原生边框、可缩放、3 个窗口按钮、无重复标题、8 个缩放热区且真实增量 40×30、5 个紧凑弹窗/面板、配置组关闭与浅色/深色对比度通过、PDF 配置未平铺、纵横溢出为 0。图片、视频、PDF 真实门禁均通过；视频包含 10 分钟与 114,842,332 B 矩阵，PDF 包含 13 个真实样本。
- 版本身份、媒体架构、类型检查、生产前端、49 文件 283/283 单元测试、主 Rust Release 核心库 384/384（10 项显式忽略）、全部集成矩阵、主库/扩展严格 Clippy、扩展 5/5 与正式无测试桥 NSIS 均通过。真实 `v1.2.3 → v1.2.4 → 卸载 → v1.2.3` 安装生命周期 50/50 通过并恢复原安装。Playwright 首轮因 Firefox 完成后清理超时未记为全量一次通过，Firefox 项目隔离复跑 6 通过、5 条件跳过、0 失败。
- 剩余发布收口顺序：提交并推送候选分支 → PR 五项 CI 全绿 → 合入 `master` → annotated `v1.2.4` → GitHub Release 与四项资产 → 回下载校验版本/大小/SHA-256/签名 → 真实公开 `v1.2.3 → v1.2.4` 更新 → README 与发布审计改为公开稳定。详细证据见 [SPECIAL_COMPRESSION_SHELL_AUDIT_1.2.4.md](SPECIAL_COMPRESSION_SHELL_AUDIT_1.2.4.md)。

## 2026-09-02 v1.2.3 正式发布关闭

- `v1.2.3` 已在公开 `v1.2.2` 基线上完成：主题化图片下拉；视频独立质量/分辨率；Windows 媒体子进程隐藏；视频探测缓存、设置防抖与事实保留；三类任务区滚动；全局任务类型；WebView 最外层 1 px 边框清理。
...
- PR #115 五项 CI 全绿后合入 `master@19a1fbe2579e52e551a6bc723b7a3c3164360468`；annotated `v1.2.3` 精确指向该提交。Release run `33598489918` 全绿，四项公开资产回下载的大小与 SHA-256 均匹配，manifest 版本、URL 和签名一致。
- 真实公开 `v1.2.2 → v1.2.3` 更新 25/25、失败 0；当前机器安装公开 `1.2.3` 于 `E:\Long\Long解压`，相关进程为 0。下一次开发必须从最新 `master`/公开 `v1.2.3` 开始。完整证据见 [RELEASE_AUDIT_1.2.3.md](RELEASE_AUDIT_1.2.3.md)。
