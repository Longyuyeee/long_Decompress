...
- 独立提交顺序为 `e7b1e82`、`05da9c3`、`8a5e2f7`、`4719c28`，均已推送到 Draft PR #81。收口自动化通过前端 200 项、类型检查、生产构建、Clippy 全目标全特性零警告、Rust 库测试 232 项通过/1 项固定机基准按设计忽略、全部测试目标编译、能力矩阵 13 项、密码状态机 6 项、真实流水线 2 项、通用格式烟雾 1 项和完整格式回环 19 项。
...
- 安装态门禁同时修正两项验证器缺陷：首次运行允许产生数据库迁移，但卸载必须保持迁移后的逐文件指纹；恢复阶段比较测试前实际菜单命令，不再把原本的开发菜单误判为必须指向安装目录。停止应用后现在等待进程完全退出，避免数据库句柄竞态。当前 P0 代码、真实样本、可见桌面和安装态证据齐全，下一步是 Draft PR 总审计；仍不在本分支直接提升版本或创建 Release。
