...
- 当前验证包括前端 190 项单测、类型检查和生产构建；Rust 全库 222/223 项通过（1 项固定机器性能基准按设计忽略），并覆盖真实临时卷探测、非空 ZIP 元数据估算及空间/介质策略，Clippy 零警告。下一步先补 Windows Release 桌面可见门禁与固定机器 I/O 基线，再决定是否增加任务级调度策略。实验性并行解压和默认并发调整仍不进入生产。
