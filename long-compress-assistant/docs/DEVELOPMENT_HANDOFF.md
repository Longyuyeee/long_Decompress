...
- 当前验证包括前端 190 项单测、类型检查、生产构建和上述真实桌面门禁；Rust 全库 222/223 项通过（1 项固定机器性能基准按设计忽略），真实临时卷探测和非空 ZIP 元数据估算通过，Clippy 零警告。下一步建立固定机器 I/O 基线，分别比较 SSD/HDD、单大文件/大量小文件和同盘/跨盘；只有稳定证据证明收益后才设计任务级策略。实验性并行解压和默认并发调整仍不进入生产。
