...
## 2026-09-23 1.3.6 历史保留与分页（最新）

- 停止保存时删除 500 条以外历史；新增每页 100 条游标加载、范围提示及查询/删除竞态处理。
- 前端 406、Rust 406 项通过，12 Rust 项按既有条件忽略；严格 Clippy、生产构建、真实历史桌面门禁通过。505 条历史为合成夹具，经真实保存/重启/翻页验证，不冒充 505 次解压。
- [场景审计](HISTORY_RETENTION_2026-09-23.md) · [发布审计](RELEASE_AUDIT_1.3.6.md) · [Release 最终资产与安装事实](https://github.com/Longyuyeee/long_Decompress/releases/tag/v1.3.6)。
- 下一步按 [1.3.6 后计划](NEXT_VERSION_AFTER_1.3.6.md) 做后端全库搜索/聚合；当前筛选仅覆盖已加载记录。旧版已删记录无法恢复，降级需先备份历史。

## 2026-09-23 1.3.5 失败后操作指引
...
## 2026-09-22 1.3.3 候选包准备（最新）

- 版本身份已升至 1.3.3；正式主程序编译通过，本机 NSIS 仍遇到已有的 makensis 启动故障，候选安装器改用精确提交的 GitHub Windows CI 产物，不使用测试桥 EXE 发布。
- 用户已明确允许关闭旧程序并备份升级。核对发现注册表为 1.1.17，实际 EXE / DLL 为 1.3.1；不能以注册表版本作为回退依据。本轮只做保留原程序和数据备份的覆盖升级，不声称重验卸载生命周期。
- 当前仍未发布；下一版方向已写入 [后续开发顺序](NEXT_VERSION_AFTER_1.3.3.md)。以下条目保留为历史过程，不代表当前阻塞状态。

## 2026-09-22 历史重试和图片跨页面状态（最新接续点）

- 历史草稿→用户启动→真实输出核对→新旧历史重启保留已通过；选择器返回路径替代范围明确。
- 真实 100 图片与 7z 并发取消/清除验证发现图片进度/控制器随页面销毁，已改为应用级隔离会话。379/379 单测、100 输出/100 历史/源哈希无变化、跨页进度回归通过。
- 详见 [发布收尾记录](RELEASE_CLOSURE_2026-09-22.md)，包含测试脚本纠偏、CI 权限欠项和后续版方向。
- 当前正式安装为 1.1.17；尚未替换用户安装，升级验收等待用户选择。公开版本仍为 1.3.2，不能提前宣称 1.3.3 发布。

## 2026-09-22 ZIP 用户链发现与收尾（最新）

- 用户允许页面自动化而非必须控制桌面；已运行真实 ZIP 往返/重启历史，并新增损坏包与完整包同队列的用户场景。
- 新场景发现截断 ZIP 仅给出英文 EOCD 错误，已在 Rust 检测路径补中文最终原因和类型化分类，保留底层详情；复测结果及范围见 [ZIP 下载失败回归](ZIP_DOWNLOAD_FAILURE_2026-09-22.md)。
- 同时修复同名冲突等待期间被提前落库为失败；正式单测 378/378、Rust 检测 7 通过/3 忽略、历史 12/12、严格 Clippy 通过。页面队列→真实引擎→保留两者→输出哈希→重启历史→可见最终原因全链通过，仅选择器路径响应由测试桥提供。
- CI 门禁提交 `02f3791` 保留在本地原分支，推送因 PAT 缺少 `workflow` 权限被拒绝。产品修复独立分支继续，不丢弃未推送工作。
- 版本仍 1.3.2，尚未发布 1.3.3；真实历史重试、媒体隔离、安装升级仍需接续，测试桥构建不得作为发行包。

## 2026-09-22 历史筛选找回与版本收尾计划（最新接续点）

- PR #138 已合入 `master@1978fd4`，此前 Windows CI 34680859485 通过；归档批量操作范围修复不再只是未合并分支。
- 本轮修复历史测试依赖真实日期的问题，保留原断言与默认 30 天规则；新增筛选无结果时重置并显示已加载记录，明确最多 500 条。正式单测 377/377、生产构建通过，真实桌面验收未完成。
- 下一建议版本 1.3.3 聚焦稳定性与易用性，仍需 CI 完整回归、真实用户链与发布安装验证。后续能力按失败处理动作、历史分页、未保存结果重启恢复、配置快照排序。
- 详细范围和验收见 [下一版本用户计划](NEXT_VERSION_USER_PLAN_2026-09-22.md)。版本未提升，未发布 Release；本机旧 EXE 不能证明本轮改动。

## 2026-09-12 归档页面任务范围（最新代码接续点）

- 修复压缩中心“取消进行中”“清除已结束”跨界操作图片/视频/PDF 的问题；任务集合和清除目标限定归档，旧任务兼容及未保存历史保护不变。
- 三个用户场景先红后绿，完整单测 375/375、生产构建通过；未把模拟命令的组件验证算作真实桌面或媒体编码验证。
- 见 [归档页面范围纠偏](ARCHIVE_CENTER_TASK_SCOPE_2026-09-12.md)。版本仍为 1.3.2，原生文件选择器暂停点继续有效，未为验收绕过或改写产品入口。
