...
## 2026-09-01 v1.1.18 候选实测后接续点（当前唯一接续点）

- 分支仍为 `codex/special-compression-navigation`，PR 仍为 [#109](https://github.com/Longyuyeee/long_Decompress/pull/109)。head `252ae03` 的 CI run `33457637334` 五项全绿；其正式 NSIS、包内 EXE/DLL 身份及六组安装生命周期已经核验，完整同轮结果为 64/64、失败 0。
...
- 当前机器已恢复公开 `1.1.17`，安装位置 `E:\Long\Long解压`，用户数据、经典菜单与自启动均恢复，相关测试进程为 0。原始同轮证据位于被忽略的 `test-results/installed-release-validation/20260901-101618`。
- 下一步只允许：提交/推送上述测试工具与审计 → 等 PR 新 head 五项 CI 全绿 → 下载新 head 的 `windows-nsis-installer` → 复核身份并重新跑六组安装生命周期。最终 head 再次通过后才合并、打 annotated `v1.1.18` 标签、验证四项公开资产、跑真实 `v1.1.17 → v1.1.18` 更新，最后把 README 从候选改为公开稳定版。
