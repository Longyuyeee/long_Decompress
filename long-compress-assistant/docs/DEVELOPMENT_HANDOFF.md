...
## 2026-09-01 v1.2.1 更新竞态修复（当前唯一接续点）
...
- 重新启动同一 1.2.0 后，四根菜单在进程内和退出后都保持，确认是时序竞态。`v1.2.1` 候选增加启动后 2 秒/6 秒延迟复核，测试增加首次完整后 8 秒稳定观察；八处版本身份和唯一 DLL 已提升到 1.2.1。
- 完整回归、无测试桥 NSIS及 `v1.2.0 → v1.2.1 → 卸载 → v1.2.0` 安装生命周期 50/50 已通过。下一步只允许 PR 五项 CI、合并/标签/Release、公开资产回下载及真实 `v1.2.0 → v1.2.1` 更新。首轮失败和修正见 [RELEASE_AUDIT_1.2.1.md](RELEASE_AUDIT_1.2.1.md)。
