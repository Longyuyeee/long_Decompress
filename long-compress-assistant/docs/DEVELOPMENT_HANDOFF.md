...
## 2026-09-01 v1.1.17 PDF 安全优化发布候选
...
- 当前仍是候选而非公开稳定版；本地全量、精确 CI NSIS 和 `v1.1.16 → 候选` 安装生命周期已完成，剩余受保护主线合入、annotated tag、四项公开资产、回下载与真实应用内更新。任一步失败均不得宣称 `v1.1.17` 已发布。详见 [RELEASE_AUDIT_1.1.17.md](RELEASE_AUDIT_1.1.17.md)。
...
- 正式安装生命周期 54/54、无测试桥 PDF 工作区 23/23；公开 `v1.1.16`、安装路径、数据、菜单和自动启动已恢复。当前严格接续点缩小为：审计提交 CI/合入 → tag/Release 四资产 → 回下载 → 公开应用内更新。
