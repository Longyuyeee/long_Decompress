...
- 真实诊断样本覆盖健康 ZIP/7Z、CRC 损坏、截断、ZIP/7Z/RAR 缺卷、加密 ZIP/7Z 缺少/错误密码、结构诊断和取消。正式合并前仍需固定 RAR 样本与 Windows 候选桌面抽查；之后进入图片受限预览或可执行任务模板阶段。
...
- 当前累计审计通过 Rust 197/198（固定机性能基准 1 项按设计忽略）、Clippy 全目标全特性零警告、前端 171 项单测、类型检查、生产构建，以及 Playwright 五类桌面/移动配置 29 项通过/6 项按浏览器策略跳过。
