...
## 2026-08-31 D-04.3 完整矩阵与安装态门禁基础（进行中）
...
- 正式 NSIS 仍是唯一未关闭项。本机重新下载后的 32 位 NSIS 工具仍以 `STATUS_DLL_NOT_FOUND` 退出，不能冒充候选成功；下一步从本分支干净 CI 下载 NSIS，再在本机执行 `RunPdfRuntimeMatrix + RunPdfWorkspaceMatrix`。
- D-04.3 未正式关闭，版本保持 `1.1.16`，不得提前发布 `1.1.17`。详见 [D04_3_PDF_FULL_MATRIX_AUDIT.md](D04_3_PDF_FULL_MATRIX_AUDIT.md)。
