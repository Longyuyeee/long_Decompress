...
- ZIP 原生解压实现已迁移到 `native_extraction/zip.rs`；密码预检会检查全部归档条目，
  ...
  `compression_service.rs` 已降至 3,347 行。
- 7Z 原生解压仍在核心服务中。CRC/密码错误分类已改为结合归档加密元数据判断，并由真实损坏包及真实加密包
  覆盖缺少、错误、正确密码；文件时间戳也会在完整写入后按选项恢复。下一步先补事务级“仅解压较新文件”、
  过滤、取消、跳过损坏和回滚回归，再迁移到独立模块。
...
2. 格式路由、暂存、资源限制、事务提交、TAR 系列、单文件流与 ZIP 引擎已经提取；7Z 的未加密损坏/
   密码分类和文件时间戳写出已完成，下一步补仅解压较新文件三态、过滤、取消、跳过损坏和回滚回归，
   再拆分 7Z 原生解压引擎；
   不在同一阶段改动事务或归档格式语义。
