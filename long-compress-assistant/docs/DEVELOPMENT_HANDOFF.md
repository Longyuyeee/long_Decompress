...
- 当前工作分支为 `agent/release-1.0.20`，版本仍为 `1.0.20`；本阶段不升版本、不创建标签、不发布 Release。
...
- 当前发布阻塞：加密 RAR 使用错误密码时，应用调用超过 60 秒仍未返回，表现为用户看到任务长期无响应。严格矩阵因此没有最终通过，不能写成“所有格式已经完成真实测试”。
- 对外格式声明已收紧：暂时移除只有引擎识别能力、但没有非空桌面闭环证据的 `ppkg`、`apm`、`scap`、`udf`、`arj`、`chm`、`z`、`taz`。以后只有补齐真实样本后才能重新公开。

### 接手后按此顺序继续

1. 定位并修复原生 RAR 解码在错误密码下不返回的问题，给密码失败与取消路径设置可验证的时间边界；不得把密码明文放进进程命令行。
2. 从头重跑 `npm.cmd run test:e2e:desktop:full-format`，确认错误密码快速失败、正确密码解压成功，并让“每个公开扩展名都有真实桌面断言”的最终对账通过。
3. 再运行完整前端测试、Rust Release 测试、Clippy、生产构建和安装生命周期检查。
4. 所有门禁通过后才将八处版本身份统一提升到 `1.0.21`，重建版本化 Shell DLL、NSIS 安装包、README 和 Release 文档，再走正式发布流程。
