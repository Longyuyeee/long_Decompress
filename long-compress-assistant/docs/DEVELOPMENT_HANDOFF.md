...
- 当前阶段的下一步不是直接自动压缩：先完成 Windows 候选桌面的手动抽查；再单独设计持久目录授权、事件合并、跨事件文件身份去重、启动/暂停/停用生命周期。即使以后启用监控，首版也只能进入现有待确认草稿，不得默认执行、带入密码或删除源文件。
...
- 当前阶段累计验证为 Rust 211 项通过、1 项固定机性能基准按设计忽略，Clippy 零警告，前端 180 项单测、类型检查和生产构建通过；Playwright 五类桌面/移动配置按 CI 单 worker 执行，32 项通过、13 项按浏览器策略跳过。模板导入、草稿确认、文件夹只读预览及压缩/解压详情覆盖 1440/1024/760/390 四档，且模拟压缩命令未被预览流程触发。
- 接手后的桌面门禁：在 Windows 候选桌面抽查“导出→预览→导入”“选择文件→计划审计→创建等待草稿”和“选择目录→只读规则预览”，确认没有自动执行、密码、删除源文件或残留监听器。
