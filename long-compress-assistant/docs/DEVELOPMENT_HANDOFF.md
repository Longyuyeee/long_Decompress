...
- 候选身份已提升为 `1.0.20-rc.2`，八处版本来源和版本化 Shell DLL 一致。不含 E2E 桥的 `rc.2` 生产 NSIS 已从
...
- `rc.1` 已安装候选的 ZIP、7Z 压缩及 7Z 快速解压完成真实界面抽查，普通与中文文件名往返哈希一致。抽查发现并修复了
  ...
  已安装版在不清理完成 ZIP 行的前提下成功生成同源 7Z；界面自动验收由用户按 Esc 主动终止，未继续操作。
...
2. 原生解压、计划内写入职责、磁盘写满、`1.0.20-rc.2` 安装生命周期及“同一来源完成 ZIP 后直接请求 7Z”已经收口；
   下一步等待 PR 门禁通过，可人工补看一次该任务的最终界面，然后合入 `master`。
