...
- 正式矩阵编排进入 `agent/io-baseline-matrix`：`performance:io-matrix` 共享同一个目标卷，顺序运行目标卷→目标卷和另一物理盘→目标卷，随后核对机器、干净 Git 提交、应用版本、规模、目标卷及预期拓扑，并汇总四项吞吐中位数、相对差异和样本范围。正式 10 样本在工作区不干净或仓库内输出未被忽略时会在采样前拒绝；汇总结论固定为 `baseline_only`，不会据此自动改变并发。
- 当前开发机已真实跑通 C→C 同卷 NVMe SSD 与 E→C 跨物理 NVMe SSD 两条 16 MiB/1000 小文件烟雾路线，均自动清理临时夹具。每场景仅 1 个样本，只能证明工具闭环，不能启用阈值；本机没有可证明 HDD，明确保持未覆盖。下一步在固定 C/E 组合积累 100 MiB/10000 小文件各 10 个样本，并在真实 HDD 机器补齐同样矩阵。只有稳定证据证明收益后才设计任务级策略，实验性并行解压和默认并发仍不进入生产。
