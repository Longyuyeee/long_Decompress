...
| D-02 前端配置 | 只有占位页；输入事实、风险和两模式配置尚未进入产品 | 当前唯一接续点 |
| D-03 执行与校验 | 未开始；没有产品转换命令 | 不得提前开放执行 |
| D-04 真实验收 | 未开始 | D-02/D-03 关闭后进行 |

### D-02 的准确开发顺序

原需求要求界面在执行前展示页数、输入大小、加密、签名、表单和附件等真实事实。现有后端只做 qpdf 运行时身份预检，因此 D-02 不能只开发前端静态卡片，应按以下顺序推进：

1. 读取现有图片/视频工作区、统一任务模型、密码脱敏和资源预检的实际代码，建立 PDF 输入候选及结构化分析结果类型。
2. 新增只读 PDF 分析后端，使用固定 qpdf 参数数组返回页数、大小、加密、签名、表单、附件及可执行性；密码不得进入日志、历史或可持久命令行文本。
...
从 D-02 的“PDF 输入候选与结构化只读分析”开始。开发前先读取 `CompressionView.vue`、图片/视频工作区、统一任务/历史、共享发布事务以及现有 `pdf_engine` 实际代码；完成一小步后更新本文件或新的节点审计，通过对应自动化与真实门禁，再提交、推送、等待 PR CI 并合入。任何偏离上述安全边界的实现应先纠偏需求文档，不得通过扩大描述掩盖代码偏移。
