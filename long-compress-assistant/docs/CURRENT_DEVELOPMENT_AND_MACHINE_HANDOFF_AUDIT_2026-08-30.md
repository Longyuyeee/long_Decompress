...
> 2026-08-31 增量状态：D-02.1 已完成产品只读 PDF 分析、密码 stdin 传递和十类真实样本对账；本文件中描述“只有 qpdf 预检/没有输入分析”的段落是 2026-08-30 基线。当前结论与下一接续点以 [D02_1_PDF_READONLY_ANALYSIS_AUDIT.md](D02_1_PDF_READONLY_ANALYSIS_AUDIT.md) 为准：UI 与转换仍冻结，下一步为 D-02.2。
...
- 当前唯一功能接续点是 **D-02 PDF 输入分析与两模式前端配置**。实际产品只注册 `preflight_pdf_engine`；压缩页中的 PDF 区域仍是占位说明，合同仍固定 `enableProductUi=false`，不存在 PDF 转换命令。这与路线要求一致，不得把 D-01 误写成可用的 PDF 压缩功能。
- 当前核心路线约完成 75%–80%。单看 PDF 阶段，D-01 至 D-04 四个大节点中关闭 1 个；按剩余实现和真实验收工作量估算约完成 25%–30%，仍有 D-02、D-03、D-04 和 `v1.1.17` 发布闭环。
...
| D-02 前端配置 | D-02.1 产品只读输入分析完成；页面仍占位，两模式和风险交互尚未进入产品 | D-02.2 是当前唯一接续点 |
| D-03 执行与校验 | 未开始；没有产品转换命令 | 不得提前开放执行 |
| D-04 真实验收 | 未开始 | D-02/D-03 关闭后进行 |

### D-02 的准确开发顺序

原需求要求界面在执行前展示页数、输入大小、加密、签名、表单和附件等真实事实。2026-08-30 基线后端只做 qpdf 运行时身份预检，因此 D-02 不能只开发前端静态卡片；D-02.1 已按以下顺序关闭前两项：
...
3. 实现“无损整理”和“兼容图片优化”配置；图片优化明确标记有损，签名默认只分析，加密文件必须正确密码后才能形成计划。
4. 默认输出新文件且不得覆盖源文件；风险必须在执行前可见，危险组合需要显式确认，不承诺一定缩小。
5. D-02 只形成真实分析与冻结配置，不把 D-03 的转换、发布或源文件处理提前塞入本节点。
...
- D-02、D-03、D-04 尚未完成；PDF 页面目前不是可用功能。
...
从 D-02.2“两模式配置与执行前风险界面”开始，消费 D-02.1 的真实结构化报告；完成一小步后更新本文件或新的节点审计，通过对应自动化与真实门禁，再提交、推送、等待 PR CI 并合入。任何偏离上述安全边界的实现应先纠偏需求文档，不得通过扩大描述掩盖代码偏移。
