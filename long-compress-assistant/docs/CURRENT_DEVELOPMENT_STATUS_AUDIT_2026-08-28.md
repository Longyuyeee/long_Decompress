...
最新接续状态：B-04.1 至 B-04.3 已完成；下一接续点为 B-04.4 统一队列与历史。完成证据见 [B04_1_IMAGE_FACT_CONTRACT_AUDIT.md](B04_1_IMAGE_FACT_CONTRACT_AUDIT.md)、[B04_2_IMAGE_STAGE_EVENT_AUDIT.md](B04_2_IMAGE_STAGE_EVENT_AUDIT.md) 与 [B04_3_IMAGE_SAFE_ORCHESTRATION_AUDIT.md](B04_3_IMAGE_SAFE_ORCHESTRATION_AUDIT.md)。
...
B-04 已开始，B-04.1 输入/输出双侧事实模型、B-04.2 真实阶段日志和 B-04.3 安全批量编排已经完成。下一接续点严格为 **B-04.4 统一队列与历史**。不能直接把图片“开始压缩”按钮改为可用：实际代码仍缺统一 task store 映射、三种终态历史和真实结果 UI。B-05 安装版真实矩阵及版本发布仍未开始。
...
| 前端执行桥缺失 | **B-04.3 已解决**：已有强类型目标规划/压缩封装和批量执行器；按钮仍按边界静态 `disabled` | B-04.4 接统一 task store，B-04.5 再由真实结果闭环开放按钮 |
...
| 批量和终态未闭环 | **B-04.3 已完成批量执行、逐图 task id、文件数进度和统一取消**；图片草稿仍未映射 task store/历史 | B-04.4 必须接统一 task store，完成、失败、取消三种终态均写入历史并跨重启读取 |
...
4. **B-04.4 队列与历史（下一步）**：复用 task store，写入 `taskType=compression`、`workloadKind=image` 和后端真实指标；验证完成/失败/取消跨重启保留。
