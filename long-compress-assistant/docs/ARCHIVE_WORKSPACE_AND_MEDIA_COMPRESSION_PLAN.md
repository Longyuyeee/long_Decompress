...
1. 为活动任务和历史记录设计向后兼容的可选 `workloadKind` 与版本化 `metrics`；旧记录缺省解释为 `archive`，数据库迁移必须可重复执行且不重写旧任务；
2. 定义媒体任务必须复用的暂存、容量预检、取消、子进程终止、冲突处理、最终校验、原子发布、Mark-of-the-Web、历史脱敏及系统回收站边界；
3. 固定候选图片、视频、PDF 引擎的项目主页、精确版本、许可证、构建来源、SHA-256、支持平台、动态/静态链接方式、安装包增量和安全更新责任；
...
- [DEVELOPMENT_ALIGNMENT_AUDIT_2026-08-26.md](DEVELOPMENT_ALIGNMENT_AUDIT_2026-08-26.md) 中的 S-00 密码存储语义与模型收口尚未完成；
