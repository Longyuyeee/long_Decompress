...
- 文件行单击只切换选择，没有“聚焦项”和“已选项”的独立语义；
- 没有双击文件夹、双击文件、键盘 Enter/Ctrl+Enter 行为；
- 没有归档内右键菜单；
...
- 本文仅完成设计审计与实施拆分，没有改变产品功能，也不触发版本提升；
- 当前正式基线仍为 `1.1.13`；
- 当前浏览中心聚焦前端测试 5/5 通过；归档相关 Rust Release 测试 38 项通过、1 项需要外部真实分卷样本而按设计忽略，相关真实格式矩阵通过；
- 本机 Windows 桌面浏览门禁因未配置 Microsoft EdgeDriver 未启动。开始节点 A 前必须补齐与当前 WebView2 匹配的驱动或在已配置的 Windows runner 上取得新的可见桌面证据；
- 下一步从 A-01 开始，不并行引入 FFmpeg、libcaesium 和 qpdf，避免把四类高风险变更混入同一版本。
