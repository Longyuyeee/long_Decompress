...
进度（2026-08-31）：**D-04.3 功能与隔离桌面矩阵通过，正式安装态待 CI 候选。** 11 类可执行 PDF × 两模式及签名/加密稳定阻断共 24 项差异为 0，人工渲染和取消/失败隔离/重启历史/默认阅读器已通过。本机 32 位 NSIS 工具链以 `STATUS_DLL_NOT_FOUND` 退出，下一步使用干净 CI 同提交候选完成正式安装生命周期；此前版本保持 `1.1.16`。证据见 [D04_3_PDF_FULL_MATRIX_AUDIT.md](D04_3_PDF_FULL_MATRIX_AUDIT.md)。
