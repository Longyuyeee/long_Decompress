...
当前 A-01 至 A-06 已全部完成并随 `v1.1.14` 发布，B-00.1 至 B-00.6 前置门禁、B-01 图片依赖/固定哈希基线、B-02 图片前端工作区、B-03 图片后端执行/发布事务、B-04 真实进度/日志/历史/UI、B-05.1 三格式矩阵和 B-05.2.1 百图真实批处理均已完成。下一接续点为 B-05.2.2 资源与故障边界；B-05 完成前不升版、不更新 Release。仍需注意：7Z 固实块和 RAR 不伪装成有界内部预览；归档内增删改继续不在大节点 A 范围内。
...
分段状态（2026-08-28）：**B-05.1 与 B-05.2.1 已完成**。B-05.1 的 JPEG、PNG、WebP 各 3 个冻结真实输入已直接通过生产 `compress_single_image`，9 个发布文件独立重新解码差异为 0；真实测试发现并修正透明 WebP 容器无效重写。B-05.2.1 在真实 Windows Release Tauri/WebView2 中以 100 个实际磁盘文件完成可见入口执行，结果为 100/100 completed、100 个唯一发布文件、100 条统一历史、JPEG/PNG/WebP 34/33/33、源 SHA-256 变化 0，耗时 17,059 ms。证据见 [B05_1_IMAGE_FORMAT_MATRIX_AUDIT.md](B05_1_IMAGE_FORMAT_MATRIX_AUDIT.md) 与 [B05_2_1_IMAGE_BATCH_AUDIT.md](B05_2_1_IMAGE_BATCH_AUDIT.md)。下一接续点为 **B-05.2.2 资源与故障边界**，B-05 整体尚未完成。
...
- B-00.1 至 B-00.6 已全部完成并通过总审计；B-01 已完成精确 feature、五个固定输入、质量/内存/并发和隔离载荷基线，B-02 已完成前端工作区、安全预览、Release/WebView2 双尺寸矩阵与真实系统文件选择，B-03 已完成图片运行时、格式/缩放/EXIF/ICC 验证、取消/容量预检、共享发布事务和 877,416 B 冻结 NSIS 增量；B-04.1 至 B-04.5 已完成事实契约、真实阶段、安全编排、跨重启三终态历史和真实结果 UI 开放；B-05.1 三格式矩阵与 B-05.2.1 百图真实批处理已完成。下一步为 B-05.2.2 超大像素、中文长路径、目标冲突、磁盘不足和取消边界；完成前不升版、不更新 Release。
