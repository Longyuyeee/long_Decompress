...
公开基线：`master` / Long解压 `1.1.13`；当前开发基线：`codex/archive-media-roadmap` / `2ce37c0`
...
当前 A-01 至 A-05.2 已补齐默认应用会话缓存、ZIP/TAR 有界文本预览、三层嵌套工作区、归档结构读取的请求级取消和 30 秒等待上限，并把浏览/嵌套/预览格式策略收敛为后端结构化能力。仍需注意：7Z 固实块和 RAR 不伪装成有界内部预览；归档内增删改继续不在大节点 A 范围内，未来若做编辑必须采用“重建新归档、完整校验、原子替换”。下一步直接进入 A-06 安装态综合矩阵。
...
- A-06 正式安装态综合矩阵已完成：候选覆盖安装后的 EXE 字节、经典右键、150% 缩放、普通 ZIP、加密 7Z/RAR、TAR/TAR.GZ、中文八层路径、混合内容、三层嵌套、危险内容、损坏包和 18 万条目取消均通过，卸载后恢复 `v1.1.13`、菜单与用户数据指纹。证据见 [ARCHIVE_WORKSPACE_A06_AUDIT.md](ARCHIVE_WORKSPACE_A06_AUDIT.md)。正式 `v1.1.14` 版本源、README、Release Notes、本地不可变 NSIS 和真实安装门禁现已完成，等待 GitHub 标签工作流生成 updater 签名与公开资产；期间不并行引入媒体引擎。
