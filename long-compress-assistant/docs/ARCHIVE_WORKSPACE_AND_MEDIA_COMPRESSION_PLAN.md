...
| `src/views/CompressionView.vue`、`src/views/SpecialCompressionView.vue` 与 `src/components/compression/` | 归档压缩队列、特殊压缩入口与媒体子工作区 | 压缩中心只保留归档；特殊压缩页面承载图片/视频/PDF 模式，媒体子组件及统一底层继续复用 |
...
> 2026-09-01 产品路线变更：以下原始界面约定已被批准替换。变更原因是归档文件压缩与媒体/PDF 专项优化的用户心智不同，需要从主导航明确分流；共享后端边界保持不变。

左侧新增一个“特殊压缩”入口，但不为三种媒体各建一个主入口。压缩中心只显示归档压缩；特殊压缩页面内部显示：

`图片压缩 | 视频压缩 | PDF 压缩`

归档和三种特殊压缩继续共享全局任务进度、历史记录、输出规划、冲突策略和清理策略；页面标题与配置面板按入口和工作负载区分。该结构仍满足“不复制任务/历史/事务系统”的原始核心需求。
...
进度（2026-08-31）：**D-02 已关闭。** D-02.1 以固定参数 qpdf 返回输入字节、页数、加密/密码、签名、表单、附件和书签事实，密码经 stdin 传递，十类真实 PDF 共 12 组预期—实际差异为 0。D-02.2 已在产品中开放“无损整理/兼容图片优化”页面内配置草稿，签名强制阻断、加密需正确密码、有损需显式确认、默认新文件且不承诺变小；真实 Windows Tauri 三类 PDF、源哈希、无任务历史和双尺寸布局差异为 0。D-03.1/D-03.2 随后关闭，D-03.3 内部发布核心也已完成；当前下一步为 D-03.3.1 低容量卷证据，产品转换命令仍冻结。证据见 [D02_1_PDF_READONLY_ANALYSIS_AUDIT.md](D02_1_PDF_READONLY_ANALYSIS_AUDIT.md)、[D02_2_PDF_RISK_CONFIGURATION_AUDIT.md](D02_2_PDF_RISK_CONFIGURATION_AUDIT.md)与 [D03_3_PDF_SAFE_PUBLICATION_AUDIT.md](D03_3_PDF_SAFE_PUBLICATION_AUDIT.md)。
...
进度（2026-08-31）：**D-03.1 已关闭。** 已建立仅供 Rust 内部使用的固定参数 qpdf 暂存执行器，复用共享容量预检，具备取消、600 秒超时、同目录受控暂存、Drop 自动清理和源 SHA-256 前后复核。真实文本/图文 PDF 两模式、签名/加密阻断与预启动取消逐项差异为 0；不注册产品命令，不发布候选，不创建任务/历史。D-03.2 候选验证随后关闭，D-03.3 内部安全发布核心也已完成；当前从 D-03.3.1 接续。证据见 [D03_1_PDF_STAGING_FOUNDATION_AUDIT.md](D03_1_PDF_STAGING_FOUNDATION_AUDIT.md)、[D03_2_PDF_CANDIDATE_VALIDATION_AUDIT.md](D03_2_PDF_CANDIDATE_VALIDATION_AUDIT.md)与 [D03_3_PDF_SAFE_PUBLICATION_AUDIT.md](D03_3_PDF_SAFE_PUBLICATION_AUDIT.md)。

进度（2026-08-31）：**D-03.2 已关闭。** 内部候选验证固定 qpdf `--check`、页数/加密/MediaBox、表单身份、注释页与类型、书签标题与目标页，以及附件名称、字节和 SHA-256；8 类 PDF × 2 模式由产品验证与独立 pypdf 双重对账，连同损坏、取消、目标竞争、变大拒绝等共 23 组差异为 0。发布、产品命令、任务和历史继续冻结；真实低容量卷仍是发布前未完成门禁。下一步严格为 D-03.3 输出锁与安全发布事务。证据见 [D03_2_PDF_CANDIDATE_VALIDATION_AUDIT.md](D03_2_PDF_CANDIDATE_VALIDATION_AUDIT.md)。
...
进度（2026-09-01）：**D-04.3、D-04 总节点与 `v1.1.17` 公开发布均已关闭。** 11 类可执行 PDF × 两模式及签名/加密稳定阻断共 24 项差异为 0；精确 `1.1.17` NSIS 以无测试桥生产 IPC 完成生命周期 54/54、PDF 工作区 23/23，正式四资产、回下载和公开 `v1.1.16 → v1.1.17` 更新 24/24 均通过。证据见 [D04_3_PDF_FULL_MATRIX_AUDIT.md](D04_3_PDF_FULL_MATRIX_AUDIT.md)与 [RELEASE_AUDIT_1.1.17.md](RELEASE_AUDIT_1.1.17.md)。

进度（2026-08-31）：**D-04.1 产品命令管线已关闭，D-04 总节点未关闭。** 单一 Tauri 命令在执行时重新核验 qpdf 与源 PDF，复用统一取消注册表和 D-03 安全发布事务，只报告转换/验证/发布事实阶段；默认拒绝变大结果，同时补齐原需求中的用户显式保留策略。真实产品命令 6 组差异为 0。任务、历史、默认阅读器和安装态矩阵仍未完成，下一步为 D-04.2，之后为 D-04.3；期间保持 `1.1.16`。证据见 [D04_1_PDF_COMMAND_PIPELINE_AUDIT.md](D04_1_PDF_COMMAND_PIPELINE_AUDIT.md)。

进度（2026-08-31）：**D-04.2 已关闭，D-04 总节点仍未关闭。** PDF 工作区已接统一任务/取消/历史、批内安全目标、发布后实测字节与页数、显式保留较大结果和默认阅读器；密码已验证的加密 PDF 因无法安全保留加密而继续阻断。真实 Windows Tauri 表单 PDF 执行/历史/默认阅读器、源哈希和双尺寸布局 4 组差异为 0。下一步严格为 D-04.3 完整内容/结构/大文件及正式安装态矩阵，期间保持 `1.1.16`。证据见 [D04_2_PDF_TASK_UI_AUDIT.md](D04_2_PDF_TASK_UI_AUDIT.md)。
