...
- 新增 Windows 11 新式右键菜单扩展基础设施，通过签名稀疏 MSIX 包和原生 `IExplorerCommand` 接入，不再依赖“显示更多选项”中的传统菜单。
- 新增“一键解压到同名文件夹”，从资源管理器直接把所选归档解压到同名目录。
- 新增“一键打包为 ZIP”，支持把单个或多个所选文件、文件夹打包到当前目录。
- 安装、卸载和应用启动时会同步右键菜单注册状态；发布流水线验证两个 Windows 11 菜单身份包的签名。
- Windows 可能缓存新安装的资源管理器扩展；首次更新后若菜单未立即出现，请重启 Windows 资源管理器或正常重启电脑。
