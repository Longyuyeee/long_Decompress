...
export const useVideoCompressionBatch = () => {
  ...
  const session = useMediaBatchSession('video')
  ...
  const runVideoBatch = async (
    ...
    if (session.isRunning.value) throw new Error('已有视频任务正在运行')
  ...
  return { runVideoBatch, cancelVideoBatch, isRunning: session.isRunning }
}
