...
describe('CompressionView', () => {
  ...
  it('keeps the compression center archive-only and exposes three media workspaces in special compression', async () => {
    ...
    expect(wrapper.get('[data-testid="image-compression-workspace"]').text()).not.toContain('图片压缩工作区')
    expect(wrapper.get('[data-testid="image-compression-workspace"]').text()).not.toContain('支持 JPG、PNG、WebP')
    expect(wrapper.get('[data-testid="image-compression-workspace"] [data-testid="image-toggle-global-settings"]').exists()).toBe(true)
    ...
    expect(wrapper.get('[data-testid="image-compression-workspace"]').text()).not.toContain('真实执行')
    ...
    expect(wrapper.get('[data-testid="video-compression-workspace"]').text()).not.toContain('视频压缩工作区')
