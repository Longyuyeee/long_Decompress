      ...
      <section v-else class="space-y-5" data-testid="history-list">
        <div v-for="group in groupedRecords" :key="group.label">
          ...
          <div class="history-panel divide-y divide-subtle/60 overflow-hidden">
            <article v-for="record in group.records" :key="record.id" class="history-row grid gap-3 p-4 cursor-pointer hover:bg-primary/5 transition-colors" tabindex="0" data-testid="history-record-row" @click="selectedRecord = record" @keydown.enter="selectedRecord = record">
              ...
              <div class="history-status-cluster">
                <span data-testid="history-status-badge" class="history-status-badge px-2.5 py-1 rounded-full border text-xs font-black" :class="terminalColor[record.status]">{{ statusLabel(record) }}</span>
                <time class="history-completed-at text-xs text-dim hidden sm:block">{{ formatDateTime(record.completedAt) }}</time>
                <i class="pi pi-chevron-right text-dim shrink-0"></i>
              </div>
            </article>
          </div>
        </div>
      ...
      <transition name="history-drawer">
        <div v-if="selectedRecord" class="fixed inset-0 z-[360] bg-slate-950/35 backdrop-blur-sm flex justify-end" @click.self="selectedRecord = null">
          <aside class="history-detail history-detail-solid h-full w-full max-w-[520px] overflow-y-auto overflow-x-hidden custom-scrollbar p-5 sm:p-7" data-testid="history-detail">
            ...
            <section v-if="recoveryGuidance" class="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4" data-testid="history-recovery-guidance" :aria-label="recoveryGuidance.title">
...
<style scoped>
.history-panel,.history-detail{background:color-mix(in srgb,var(--bg-card) 92%,transparent);border:1px solid var(--border-subtle);box-shadow:0 18px 45px rgba(22,30,55,.08);backdrop-filter:blur(24px)}
.history-panel{border-radius:1.5rem}.history-detail{background:var(--bg-modal);backdrop-filter:none}.history-detail-solid{box-shadow:-18px 0 48px rgba(15,23,42,.2)}
.history-kpi{min-width:0;padding:1.05rem;border-radius:1.35rem;border:1px solid var(--border-subtle);background:linear-gradient(145deg,color-mix(in srgb,var(--bg-card) 94%,transparent),color-mix(in srgb,var(--dynamic-accent) 4%,var(--bg-card)));box-shadow:0 12px 28px rgba(20,30,60,.06)}
.history-kpi-primary{background:linear-gradient(145deg,color-mix(in srgb,var(--dynamic-accent) 16%,var(--bg-card)),var(--bg-card))}.history-kpi>div{display:flex;justify-content:space-between;gap:.5rem;color:var(--text-muted);font-size:.7rem;font-weight:900;text-transform:uppercase;letter-spacing:.1em}.history-kpi strong{display:block;font-size:2rem;line-height:1.05;margin-top:.75rem;color:var(--text-base);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.history-kpi em{font-size:.85rem;font-style:normal;color:var(--text-muted);margin-left:.15rem}.history-kpi small{display:block;color:var(--text-dim);font-size:.65rem;margin-top:.4rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.history-ghost-button{min-height:2.6rem;padding:.65rem .9rem;border:1px solid var(--border-subtle);border-radius:.85rem;background:var(--bg-input);display:flex;align-items:center;gap:.5rem;font-size:.75rem;font-weight:900;transition:.2s}.history-ghost-button:hover:not(:disabled){border-color:color-mix(in srgb,var(--dynamic-accent) 45%,var(--border-subtle));color:var(--dynamic-accent)}.history-ghost-button:disabled{opacity:.45}
.history-control{height:2.75rem;min-width:0;border:1px solid var(--border-subtle);border-radius:.9rem;background:var(--bg-input);color:var(--text-base);font-size:.75rem;font-weight:800;padding:.65rem .85rem;outline:none}.history-control:focus{border-color:var(--dynamic-accent);box-shadow:0 0 0 3px color-mix(in srgb,var(--dynamic-accent) 12%,transparent)}
.history-chart{background:repeating-linear-gradient(to bottom,transparent 0,transparent calc(33.33% - 1px),var(--border-subtle) 33.33%);border-radius:1rem;padding:.5rem .2rem 0}.history-row{grid-template-columns:minmax(0,1.7fr) minmax(9rem,.75fr) minmax(5.5rem,.45fr) minmax(15rem,auto);align-items:center}.history-chip{padding:.3rem .55rem;border-radius:.55rem;background:var(--bg-input);border:1px solid var(--border-subtle);font-weight:800}.history-status-cluster{display:grid;grid-template-columns:auto minmax(7.6rem,auto) auto;align-items:center;justify-content:end;gap:.75rem;min-width:0}.history-status-badge{display:inline-flex;align-items:center;justify-content:center;line-height:1;white-space:nowrap;word-break:keep-all;flex:none}.history-completed-at{white-space:nowrap;font-variant-numeric:tabular-nums;text-align:right}
.history-icon-button{width:2.6rem;height:2.6rem;flex:none;border-radius:.9rem;border:1px solid var(--border-subtle);background:var(--bg-input);color:var(--text-muted)}.detail-metric,.detail-section{border:1px solid var(--border-subtle);background:var(--bg-input);border-radius:1.1rem;padding:1rem;min-width:0}.detail-metric span{display:block;color:var(--text-muted);font-size:.68rem;font-weight:900;text-transform:uppercase;letter-spacing:.08em}.detail-metric strong{display:block;margin-top:.45rem;font-size:.9rem;overflow:hidden;text-overflow:ellipsis}.detail-section{margin-top:.75rem}.detail-section h3{font-size:.75rem;font-weight:900;display:flex;align-items:center;gap:.55rem}.detail-path{display:block;width:100%;font-size:.72rem;color:var(--text-muted);white-space:pre-wrap;overflow-wrap:anywhere;background:color-mix(in srgb,var(--bg-card) 70%,transparent);border-radius:.65rem;padding:.65rem}.detail-log{display:grid;grid-template-columns:5.5rem minmax(0,1fr);gap:.7rem;font-size:.7rem}.detail-log time{color:var(--text-dim)}.detail-log span{overflow-wrap:anywhere}
...
@media(max-width:1050px){.history-row{grid-template-columns:minmax(0,1fr) minmax(5.5rem,auto) minmax(15rem,auto)}.history-row>div:nth-child(2){display:none}}
@media(max-width:700px){.history-row{grid-template-columns:minmax(0,1fr) minmax(7rem,auto)}.history-row>div:nth-child(3){display:none}.history-status-cluster{grid-template-columns:auto auto}.history-completed-at{display:none}.history-kpi{padding:.85rem}.history-kpi strong{font-size:1.45rem}.history-kpi small{display:none}}
