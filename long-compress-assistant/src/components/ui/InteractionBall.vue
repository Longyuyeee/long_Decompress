...
const needsAttention = computed(() => {
  ...
  return (lastLog.includes('密码') || lastLog.includes('password')) && activeTask.value?.progress === 0
})
</script>
    ...
    <div v-if="taskStore.activeTaskCount > 0"
      ...
      >
        ...
        <div class="relative z-10 flex flex-col flex-1 min-w-0 opacity-0 group-hover:opacity-100 transition-all duration-500 -translate-x-4 group-hover:translate-x-0 overflow-hidden pr-2">
          <div class="text-[0.5rem] text-muted uppercase font-black tracking-widest truncate">{{ activeTask?.name }}</div>
          <div class="flex items-center justify-between gap-4 mt-0.5">
            <div class="text-[0.625rem] text-content font-black">{{ totalProgress }}%</div>
            <div class="text-[0.4375rem] font-bold uppercase tracking-tighter" :style="{ color: 'var(--dynamic-accent)' }">
              {{ needsAttention ? appStore.t('progress.waiting') : appStore.t('progress.processing') }}
            </div>
          </div>
        </div>
      </div>
