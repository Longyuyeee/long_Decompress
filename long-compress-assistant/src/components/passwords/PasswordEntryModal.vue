  ...
  >
    <div class="modal-content space-y-4 bg-modal text-content p-1">
      ...
      <div class="space-y-1.5">
        <label class="text-[0.5625rem] font-black text-muted uppercase tracking-widest ml-1">{{ appStore.t('vault.column.name') }} *</label>
      ...
      <div class="space-y-1.5 relative">
        <label class="text-[0.5625rem] font-black text-muted uppercase tracking-widest ml-1">{{ appStore.t('vault.column.password') }} *</label>
      ...
      <!-- 备注说明 -->
      <div class="space-y-1.5">
        <label class="text-[0.5625rem] font-black text-muted uppercase tracking-widest ml-1">{{ appStore.t('vault.column.notes') }}</label>
        <textarea v-model="form.notes" rows="2" :placeholder="appStore.t('vault.placeholder.notes')" class="w-full bg-input border border-subtle rounded-xl px-4 py-2.5 text-[0.625rem] text-muted focus:border-primary transition-all resize-none shadow-sm"></textarea>
      </div>

      <!-- 交互按钮 -->
      <div class="pt-2 flex gap-2">
        <button @click="emit('update:visible', false)" class="flex-1 py-2.5 rounded-xl bg-input border border-subtle text-muted text-[0.5625rem] font-black uppercase hover:text-content transition-all tracking-widest">{{ appStore.t('vault.confirm.cancel') }}</button>
        <button @click="handleSave" :disabled="isSaving || !form.name || !form.password" class="flex-[2] py-2.5 rounded-xl bg-primary text-white text-[0.5625rem] font-black shadow-lg shadow-primary/20 hover:brightness-110 flex items-center justify-center gap-2 transition-all tracking-widest">
          <i v-if="isSaving" class="pi pi-spin pi-spinner text-[0.5rem]"></i>
