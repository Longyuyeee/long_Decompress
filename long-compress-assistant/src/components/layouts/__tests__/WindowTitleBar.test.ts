...
describe('WindowTitleBar', () => {
  ...
  it('exposes accessible controls and forwards each native window action', async () => {
    ...
    expect(mocks.invoke).toHaveBeenCalledWith('start_native_window_drag')
    ...
    expect(minimize.attributes()).not.toHaveProperty('data-tauri-drag-region')
    expect(maximize.attributes()).not.toHaveProperty('data-tauri-drag-region')
    expect(close.attributes()).not.toHaveProperty('data-tauri-drag-region')
