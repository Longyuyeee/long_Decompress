...
describe('MainLayout Component', () => {
  ...
  it('displays correct app title and description', () => {
    expect(wrapper.text()).toContain('胧压缩·方便助手')
  ...
  it('toggles theme when theme button is clicked', async () => {
    ...
    expect(themeButton.attributes('title')).toBe('切换到暗色主题')
    ...
    expect(themeButton.attributes('title')).toBe('切换到亮色主题')
  ...
  it('shows notification count badge', () => {
    ...
    expect(badge.text()).toBe('2') // 初始有2个通知
  })
  ...
  it('displays user menu dropdown when showUserMenu is true', async () => {
    ...
    expect(userMenu.text()).toContain('管理员')
    expect(userMenu.text()).toContain('设置')
    expect(userMenu.text()).toContain('关于')
    expect(userMenu.text()).toContain('退出登录')
  })
  ...
  it('formats notification time correctly', () => {
    ...
    // 第一个通知是1小时前
    expect(wrapper.vm.formatRelativeTime(notifications[0].createdAt)).toContain('小时前')

    // 第二个通知是30分钟前
    expect(wrapper.vm.formatRelativeTime(notifications[1].createdAt)).toContain('分钟前')
  })
  ...
  it('handles logout function', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('退出登录')
    expect(wrapper.vm.showUserMenu).toBe(false)
  })
  ...
  it('has search functionality', () => {
    ...
    expect(searchInput.attributes('placeholder')).toBe('搜索文件或设置...')
  })
