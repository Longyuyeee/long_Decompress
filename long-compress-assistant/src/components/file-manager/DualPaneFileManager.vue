...
const notice = ref('')
const context = ref<{ pane: PaneState; entry: LocalFileEntry; x: number; y: number } | null>(null)
...
const createPane = (id: 'left' | 'right'): PaneState => reactive({ id, path: '', entries: [], selected: new Set<string>(), history: [], historyIndex: -1, loading: false, error: '' })
...
const toggle = (pane: PaneState, entry: LocalFileEntry, event: MouseEvent) => {
  activePane.value = pane.id
  const next = new Set(event.ctrlKey ? pane.selected : [])
  if (event.ctrlKey && next.has(entry.path)) next.delete(entry.path)
...
onUnmounted(() => window.removeEventListener('pointerdown', handleGlobalPointer))
  ...
  <div class="file-manager h-full min-w-0 flex flex-col gap-3 p-responsive p-6" data-testid="dual-pane-file-manager">
    ...
    <main class="pane-grid flex-1 min-h-0">
      <section v-for="pane in [left, right]" :key="pane.id" class="file-pane" :class="{ active: activePane === pane.id }" @pointerdown="activePane = pane.id">
        ...
        <div class="path-strip" :title="pane.path"><i class="pi pi-folder-open"></i><strong>{{ pane.path }}</strong><span>{{ pane.entries.filter(item => item.isDir).length }} 文件夹 · {{ pane.entries.filter(item => !item.isDir).length }} 文件</span></div>
        <div class="file-head"><span>名称</span><span>大小</span><span>修改时间</span></div>
        <div class="file-list custom-scrollbar">
          <button v-for="entry in pane.entries" :key="entry.path" class="file-row" :class="{ selected: pane.selected.has(entry.path) }" :data-path="entry.path" @click="toggle(pane, entry, $event)" @dblclick="activate(pane, entry)" @contextmenu="openContext(pane, entry, $event)">
        ...
        <footer>{{ pane.selected.size ? `已选择 ${pane.selected.size} 项` : '单击选择 · Ctrl 多选 · 双击打开 · 右键操作' }}</footer>
      </section>
    </main>
    <footer class="transfer-bar shrink-0">
      ...
      <button :disabled="!activeSelectionCount || busy" @click="runTransfer('move', activePane === 'left' ? left : right)"><i class="pi pi-arrow-right-arrow-left"></i>移动到另一栏</button>
    ...
    <Teleport to="body">
      <section v-if="context" class="file-context" :style="{ left: `${context.x}px`, top: `${context.y}px` }" @pointerdown.stop @contextmenu.prevent>
        <header><i :class="icon(context.entry)"></i><strong class="truncate">{{ context.entry.name }}</strong></header>
        ...
        <button @click="runTransfer('copy', context.pane, context.entry)"><i class="pi pi-copy"></i><span>复制到另一栏 →</span></button>
        <button @click="runTransfer('move', context.pane, context.entry)"><i class="pi pi-arrow-right-arrow-left"></i><span>移动到另一栏 →</span></button>
