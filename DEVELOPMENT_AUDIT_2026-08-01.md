...
当前 `master` 可以作为稳定开发基线，v1.0.21 仍是应当面向用户分发的最新正式版本。本轮不创建 v1.0.22：
v1.0.21 标签之后只有公开更新验收脚本和发布文档变更，没有产品运行时代码差异。仅修改版本号并重新打包不能给用户
带来新能力，反而会增加一次无意义的签名更新、安装和回滚路径。

发布判断：**当前版本健康，v1.0.22 暂缓（NO-GO），待形成可验证的产品增量后再发布。**
...
- v1.0.21 安装包无需重新上传或覆盖；公开 Release 与验收报告已经闭环。
- 不移动 `v1.0.21` 标签，不覆盖已发布资产，不创建内容为空的 v1.0.22。
- 当下一批产品代码完成并通过完整 CI 后，再统一升级八个版本源、构建唯一版本化 Shell DLL、生成 NSIS/updater 资产并发布新的补丁版本。
